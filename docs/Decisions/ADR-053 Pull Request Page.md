...
- **Refresh**: on session open, when a new `pr-link` lands, every 5 minutes for open PRs of open tabs, and on demand. *(Superseded by [[ADR-127 The PR Panel Refreshes On Events Not On A Timer]]: a push and the end of a turn drive reads, and the interval follows what is on screen; five minutes is now only the backstop.)*
