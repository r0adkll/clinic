...
/// Places a tab's persistent content view into the SwiftUI hierarchy and syncs panel/page state.
struct TabContentRepresentable: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> TabContentView {
        let v = tab.contentView
        return v
    }

    func updateNSView(_ v: TabContentView, context: Context) {
        v.setPanel(tab.panelVisible ? tab.panelSurface : nil)
        let (page, minWidth) = pageContent()
        v.setPage(page, minWidth: minWidth)
        if isSelected { DispatchQueue.main.async { if tab.surface.window?.firstResponder !== tab.surface, tab.surface.window != nil, !(tab.panelVisible) { tab.surface.window?.makeFirstResponder(tab.surface) } } }
    }
    ...
    private func pageContent() -> (AnyView?, CGFloat) {
